...
\item \cs{SyntaxGenerator}
\end{itemize}

\subsubsection{Example}

To transform entities for the running example using the Roslyn \cs{SyntaxTree} \ac{API}, the code could look like the following.
...
\inputminted[firstline=16,lastline=91]{csharp}{samples/Roslyn/Program.cs}

Notice how immutability makes transforming code harder by requiring the use of methods such as \cs{ReplaceNodes} and how the level of detail leads to very verbose code, in some cases requiring even the specification of individual semicolons.

\medskip

\todo{Explain \cs{SyntaxGenerator} example follows}

\inputminted[firstline=18,lastline=107]{csharp}{samples/Roslyn.SyntaxGenerator/Program.cs}

\todo{Comment on the preceding example}
