...
\cs{CSharpE.Logging} is a simple built-time-only extension that logs method name and method arguments to the console at the start of each method in a project.

This extension can be compared with PostSharp Diagnostics, \cite{postsharp-diag} which serves a similar purpose. While PostSharp Diagnostics has vastly more features, the extension is only 60 lines of code. This shows that simple custom transformations can be implemented very quickly and might have the potential to replace commercial tools.

\todo{update number of lines if i switch to string interpolation}
