...
\item The Microsoft C\# specification calls the member that is invoked after an instance is garbage collected "destructor". But the Ecma C\# specification\footnotemark{} and the official documentation~\cite{finalizer} both call this member "finalizer", so that is the name used here.
