...
\item The \ac{API} includes implicit conversion operators when appropriate. Specifically, they can be used to convert from a definition to a reference (e.g. from \cs{TypeDefinition} to \cs{IdentifierExpression} \linebreak[0] \todo[inlinewidth=7.75cm, noinlinepar]{why not \cs{NamedTypeReference}?}) or from a node to a simple wrapper for that node (e.g. from \cs{Expression} to \cs{ExpressionStatement}).
